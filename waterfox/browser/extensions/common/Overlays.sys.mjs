...
export class Overlays {
   ...
   async load(urls) {
       ...
       for (const url of unloadedOverlays) {
         ...
         for (let i = 0, len = styledNodes.snapshotLength; i < len; ++i) {
           ...
           // Re-assigning to node.style.display to itself can sometimes force a style recalculation.
           // To satisfy linters and achieve a similar effect:
           node.style.setProperty("display", node.style.getPropertyValue("display"));
         }
         ...
         for (const o of t_unloadedOverlays) {
           unloadedOverlays.add(o);
         }
  ...
  _insertElement(parent, node) {
    ...
    if (!wasInserted) {
      // position is 1-based
      const position = Number.parseInt(node.getAttribute("position"), 10);
   ...
   */
   _mergeElement(target, node, shadow = false) {
       ...
       for (const childNode of node.children) {
         if (childNode.localName === "script") {
           this.deferredLoad.push(...this.loadScript(childNode));
         }
       }

       let sourceNodeForChildren = node;
       // Get the DocumentFragment
       if (node.localName === "template") {
         sourceNodeForChildren = node.content;
       }

       for (let i = 0, len = sourceNodeForChildren.childElementCount; i < len; i++) {
         const child = sourceNodeForChildren.firstElementChild;
         child.remove();
   ...
   loadScript(node) {
       ...
       if (this.document.readyState === "complete") {
         this.window.addEventListener = function (
           type,
           listener,
           useCapture, // This is the original parameter
           ...args
         ) {
           if (type === "load") {
             let processedUseCapture = useCapture; // Local variable to hold the processed value

             if (typeof processedUseCapture === "object" && processedUseCapture !== null) {
               // Handle cases where useCapture might be an options object
               processedUseCapture = processedUseCapture.capture;
             }

             if (typeof processedUseCapture === "undefined") {
               processedUseCapture = true; // Default to capture phase if undefined
             }
             deferredLoad.push({ listener, useCapture: processedUseCapture });
             return null;
           }
           // For non-"load" events, pass the original useCapture argument
